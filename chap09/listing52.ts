...
import ray, { Ray } from './lib/ray.ts'
...
import { lambertian, metal } from './lib/material.ts'
...
function main () {
  ...
  const material_center = lambertian(color(0.7, 0.3, 0.3))
  const material_left = metal(color(0.8, 0.8, 0.8), 0.3)
  const material_right = metal(color(0.8, 0.6, 0.2), 1.0)
