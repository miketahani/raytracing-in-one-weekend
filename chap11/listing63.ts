...
import { lambertian } from './lib/material.ts'
...
function main () {
  ...
  const material_left = lambertian(color(0, 0, 1))
  const material_right = lambertian(color(1, 0, 0))

  world.add(sphere(point3(-R, 0, -1), R, material_left))
  world.add(sphere(point3( R, 0, -1), R, material_right))

  // Camera
  const cam = new Camera(90.0, aspect_ratio)
